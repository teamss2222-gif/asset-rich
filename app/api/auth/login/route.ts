...
export async function POST(request: Request) {
  try {
    ...
    if (!result.rowCount) {
      return apiError({ status: 401, code: "INVALID_CREDENTIALS", message: "아이디 또는 비밀번호가 올바르지 않습니다." });
    }
    ...
    return apiOk({ username: user.username }, { code: "LOGIN_OK", message: "login success" });
  } catch (error) {
    const message = error instanceof Error ? error.message : "로그인 처리 중 오류가 발생했습니다.";
    return apiError({ status: 500, code: "LOGIN_FAILED", message });
  }
}
